...
public class Common : BaseEmployee
{
    ...
    public JsonResult insertusername(Login log)
    {
        requestModel = CreateRequestModel(this.userParameter, log, StoredProcedureNames.insertloginprocedures);
        responseResult = localResponse.SaveItems(requestModel);
        return new JsonResult(responseResult);

    }
    public JsonResult Get_EmployeesLogin(String username)
    {
        loginform log = new loginform();
        log.username = username;
        requestModel = CreateRequestModel(this.userParameter,log, StoredProcedureNames.getuser);
        responseResult = localResponse.GetItems(requestModel);
        if (responseResult.Data.Count == 0 || responseResult.Data == null)
        {
            responseResult.Exception = "No Data Found";
            return new JsonResult(responseResult);
    ...
    public JsonResult GetAllEmployeeLogin()
    {
        requestModel = CreateRequestModel(this.userParameter, null, StoredProcedureNames.getloginprocedures);
        responseResult = localResponse.GetItems(requestModel);
        if (responseResult.Data.Count == 0 || responseResult.Data == null)
        {
            responseResult.Exception = "No Data Found";
    ...
    public JsonResult RegisterTasks(Tasks tk)
    {
        Console.WriteLine(tk.DueDate);
        requestModel = CreateRequestModel(this.userParameter, tk, StoredProcedureNames.inserttaskprocedure);
        responseResult = localResponse.SaveItems(requestModel);
        return new JsonResult(responseResult);

    }
    public JsonResult GetAllTasks(TasksId tid)
    {
        requestModel = CreateRequestModel(this.userParameter, tid, StoredProcedureNames.getalltasksprocedure);
        responseResult = localResponse.GetSetItems(requestModel);
        if (responseResult.Data.Count == 0 || responseResult.Data == null)
        {
            responseResult.Exception = "No Data Found";
            return new JsonResult(responseResult);
